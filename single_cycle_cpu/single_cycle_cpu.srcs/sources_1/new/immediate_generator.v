`timescale 1ns / 1ps

`timescale 1ns / 1ps
//
// RV32I Immediate Generator
// Extracts and sign-extends immediate values based on instruction format
//

module immediate_generator(
    input  [31:0] instr,      // full 32-bit instruction
    input  [2:0]  ImmSrc,     // selects immediate type
    output reg [31:0] imm_out // sign-extended immediate
);

    always @(*) begin
        case (ImmSrc)

            // I-type immediate (ADDI, LW, JALR, ANDI, etc.)
            3'b000: imm_out = {{20{instr[31]}}, instr[31:20]};

            // S-type immediate (SW)
            3'b001: imm_out = {{20{instr[31]}}, instr[31:25], instr[11:7]};

            // B-type immediate (BEQ, BNE, BLT, BGE...)
            3'b010: imm_out = {{19{instr[31]}},
                                instr[31], instr[7],
                                instr[30:25], instr[11:8],
                                1'b0};

            // U-type immediate (LUI, AUIPC)
            3'b011: imm_out = {instr[31:12], 12'b0};

            // J-type immediate (JAL)
            3'b100: imm_out = {{11{instr[31]}},
                                instr[31], instr[19:12],
                                instr[20], instr[30:21],
                                1'b0};

            default: imm_out = 32'b0;
        endcase
    end

endmodule
